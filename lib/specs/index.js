...
class Specs {
  ...
  _buildSpecsForRoles() {
    ...
    _.forEach(this.roles, (features, roleId) => {
      ...
      this._filterSecurity(spec)

      spec.tags       = this._buildTags(spec)
      spec.info.title = roleId
  ...
  _buildSecurity() {
    return _.map(this.swagger.spec.securityDefinitions, (val, key) => {
      const def = {} ; def[key] = [] ; return def
    })
  }

  _filterSecurity(spec) {
    const publicOperationIds = this.features.publicFunctionality.operationIds
    const security = this._buildSecurity()
