...
class Specs {
  constructor({ swagger, services, features, roles }, masterSpecPath = null) {
    ...
    if (!this.swagger && !this.services && !this.features && !this.roles) {
      if (!this.masterSpecPath) {
        this.masterSpecPath = `${rootPath}/src/api/swagger.json`
      }

      const spec         = require(this.masterSpecPath)
      const operationIds = this._getOperationIds(spec)

      this.features = { default: { operationIds } }
  ...
  getSpec(roleIds) {
    if (_.isArray(roleIds)) {
      return this._mergeSpecs(roleIds)
    }

    return _.assign({}, this.specsByRole[roleIds])
  }

  getOperationIds(roleId) {
    const spec = this.getSpec(roleId)
    if (spec) {
      return this._getOperationIds(spec)
    }
    return []
  }

  _getOperationIds(spec) {
    let operationIds   = []
  ...
  _mergeSpecs(roleIds) {
    const roleId     = roleIds.pop()
    const mergedSpec = _.assign({}, this.specsByRole[roleId])

    _.forEach(roleIds, roleId => {
      const spec        = this.specsByRole[roleId]
      const info        = mergedSpec.info
      const tags        = mergedSpec.tags
      const paths       = mergedSpec.paths
      const definitions = mergedSpec.definitions

      _.assign(tags, spec.tags)
      ...
      for (let path in spec.paths) {
        ...
        if (!paths[path]) {
          ...
          _.assign(paths[path], methods)
        }
      }

      _.assign(definitions, spec.definitions)

      info.title = `${info.title}, ${spec.info.title}`
    })
